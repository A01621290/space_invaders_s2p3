...
class Entity{
    ...
    public:
        ...
        void Draw(const X11 &x11, unsigned int Frame);            //Method to draw each class' assets
        bool operator& (const Entity& b);             //The & operator will check for collisions between entities
        void Print(void);

        virtual bool Collision(Entity& b) = 0;        /*Determines what the entity will do when colliding with
                                                        another entity*/
...
class Block final : public Entity{
    public:
        ...
        bool Collision(Entity& b) override;
};
